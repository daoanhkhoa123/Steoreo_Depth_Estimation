...
def rectification(img1, img2, pts1, pts2, F):
    ...
    valid, H1, H2 = cv2.stereoRectifyUncalibrated(np.float32(pts1), np.float32(pts2), F, imgSize=(w1, h1))
    if not valid:
        raise ValueError("Stereo rectification failed. Check the fundamental matrix and matching points.")
