...
displ = left_matcher.compute(image_left, image_right)  # .astype(np.float32)/16
dispr = right_matcher.compute(
    image_right, image_left)  # .astype(np.float32)/16
displ = np.int16(displ)
dispr = np.int16(dispr)
# important to put "imgL" here!!!
filteredImg = wls_filter.filter(displ, image_left, None, dispr)
filteredImg = cv2.normalize(
    src=filteredImg, dst=filteredImg, beta=0, alpha=255, norm_type=cv2.NORM_MINMAX)
filteredImg = np.uint8(filteredImg)


def depth_map(imgL, imgR, window_size=5):
